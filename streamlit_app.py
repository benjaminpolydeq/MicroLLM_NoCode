...
st.markdown(
    f"""
    <div style="background:linear-gradient(135deg,#1e3c72,#667eea);padding:2rem;border-radius:12px;color:white;">
...
for msg in st.session_state.messages:
    if msg["role"] == "user":
        st.markdown(f"<div style='background:#667eea;color:white;padding:1rem;border-radius:12px;margin:1rem 0 1rem 20%;'>{msg['content']}</div>", unsafe_allow_html=True)
    else:
        st.markdown(f"<div style='background:#f4f6f8;padding:1rem;border-radius:12px;margin:1rem 20% 1rem 0;border-left:4px solid #667eea;'>{msg['content']}</div>", unsafe_allow_html=True)
